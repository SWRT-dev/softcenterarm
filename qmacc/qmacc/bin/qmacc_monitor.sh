...
#URL_PREFIX="http://"
#DOWNLOAD_URL="xnews.sparta.html5.qq.com/router/text?type="
#UNINSTALL_DOWNLOAD_URL="xnews.sparta.html5.qq.com/router/uninstall/text?type="
...
xiaomi_download_url_init() {
    ...
    if [ ! -z "${mips}" ];then
        DOWNLOAD_URL="${URL_PREFIX}${DOWNLOAD_URL}xiaomi"
        UNINSTALL_DOWNLOAD_URL="${URL_PREFIX}${UNINSTALL_DOWNLOAD_URL}xiaomi"
    else
        DOWNLOAD_URL="${URL_PREFIX}${DOWNLOAD_URL}xiaomi-arm"
        UNINSTALL_DOWNLOAD_URL="${URL_PREFIX}${UNINSTALL_DOWNLOAD_URL}xiaomi-arm"
    fi
...
hiwifi_download_url_init() {
    DOWNLOAD_URL="${URL_PREFIX}${DOWNLOAD_URL}hiwifi"
...
asuswrt_download_url_init() {
    ...
    DOWNLOAD_URL="${URL_PREFIX}${DOWNLOAD_URL}asuswrt-merlin"
...
check_backtar_file() {
    ...
    if [ ! -e "${backtar}" ];then
        ...
        echo 'check_backtar_file tar before' 
        local exefile="${RUNNING_DIR}/${PLUGIN_EXE}"
        tar zxvf "${runtar}" -C "${RUNNING_DIR}"
        [ "$?" != "0" ] && return
        echo 'check_backtar_file tar 1'
        chmod u+x "${exefile}"
        ${exefile} >/dev/null 2>&1 &
		[ "$?" != "0" ] && return
        ...
        if [ "$?" = "0" ];then
            cp "${runtar}" "${backtar}"
			echo 'check_backtar_file cp backtar!!' 
            if [ "$?" != "0" ]; then
                [ -f "${backtar}" ] && rm "${backtar}"
                [ -f "${runtar}" ] && rm "${runtar}"
                return
