...
start_vpn() {
	...
	cat <<-EOF >$CONFIG
		...
		mtu=$shadowvpn_mtu
		intf=$shadowvpn_tun
		up=/jffs/softcenter/scripts/shadowvpn_client_up.sh
		down=/jffs/softcenter/scripts/shadowvpn_client_down.sh
	...
	start_speeder
	/jffs/softcenter/bin/shadowvpn -c $CONFIG -s start
}
...
start_dns() {
	...
	if [ -z "$Pcap_DNSProxy" ]; then
		...
		sed -i '/^Listen Port/c Listen Port = 7913' /jffs/softcenter/ss/dns/Config.ini
		/jffs/softcenter/ss/dns/dns.sh > /dev/null 2>&1 &
	fi
...
if [ "$shadowvpn_start" == "1" ]; then
	...
	if [ -z "$startshadowvpn" ];then
	   if [ ! -f /jffs/softcenter/scripts/shadowvpn.sh ]; then
	      sed -i '2a sh /usr/bin/shadowvpn.sh' /jffs/scripts/wan-start
	   else
	      sed -i '2a sh /jffs/softcenter/scripts/shadowvpn.sh' /jffs/scripts/wan-start
	   fi
	fi
...
stop_vpn() {
	if [ ! -z "$shadowvpn" ]; then
		...
		/jffs/softcenter/bin/shadowvpn -c $CONFIG -s stop >/dev/null 2>&1
	fi
	# kill Pcap_DNSProxy
	if [ ! -z "$Pcap_DNSProxy" ]; then
