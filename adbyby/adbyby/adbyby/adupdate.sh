...
rm -f /jffs/softcenter/adbyby/data/*.bak

touch /tmp/local-md5.json && md5sum /jffs/softcenter/adbyby/data/lazy.txt /jffs/softcenter/adbyby/data/video.txt > /tmp/local-md5.json
...
if [ "$lazy_online"x != "$lazy_local"x -o "$video_online"x != "$video_local"x ]; then
    ...
    if [ "$lazy_online"x == "$lazy_local"x -a "$video_online"x == "$video_local"x ]; then
      ...
      mv /tmp/lazy.txt /jffs/softcenter/adbyby/data/lazy.txt
      mv /tmp/video.txt /jffs/softcenter/adbyby/data/video.txt
