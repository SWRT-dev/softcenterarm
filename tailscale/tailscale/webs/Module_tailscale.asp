...
var params_chk = ['tailscale_enable', 'tailscale_ipv4_enable', 'tailscale_ipv6_enable', 'tailscale_advertise_routes', 'tailscale_accept_routes', 'tailscale_exit_node', 'tailscale_auto_update'];
var	refresh_flag;
var count_down;
var myid;
var dbus = {};
...
function get_tcnets_status(){
	var id = parseInt(Math.random() * 100000000);
	var postData = {"id": id, "method": "tailscale_tsnets.sh", "params":[1], "fields": ""};
	$.ajax({
		type: "POST",
		cache: false,
		url: "/_api/",
...
function save(){
	...
	var id = parseInt(Math.random() * 100000000);
	var postData = {"id": id, "method": "tailscale_config.sh", "params": ["web_submit"], "fields": dbus_new};
	$.ajax({
		type: "POST",
		url: "/_api/",
		data: JSON.stringify(postData),
...
function tailscale_netcheck() {
	...
	var id = parseInt(Math.random() * 100000000);
	var postData = {"id": id, "method": "tailscale_ncheck.sh", "params":[], "fields": ""};
	$.ajax({
		type: "POST",
		cache:false,
		url: "/_api/",
...
function write_netcheck() {
	$.ajax({
		...
		dataType: 'text',
		success: function(res) {
			if (res.search("Report") != -1)
				$('#ts_check').val(res);
			else 
				setTimeout("write_netcheck();", 500);
...
function open_alist_hint(itemNum) {
	...
	if (itemNum == 4) {
		width = "780px";
		statusmenu = "&nbsp;&nbsp;&nbsp;&nbsp;<b>自动更新</b>：开启该功能后，可以自动更新tailscale客户端到最新稳定版<br/><br/>"
		_caption = "自动更新";
		return overlib(statusmenu, OFFSETX, 20, OFFSETY, -330, RIGHT, STICKY, WIDTH, 'width', CAPTION, _caption, CLOSETITLE, '');
	}
