...
INI_FILE=/jffs/softcenter/configs/frps.ini
...
onstart() {
	...
	if [ -z "${frps_client_version}" ];then
		...
		frps_client_version=$(/jffs/softcenter/bin/frps --version)
	fi
	echo_date "当前插件frps主程序版本号：${frps_client_version}"

	# frps配置文件
	echo_date "生成frps配置文件到 /jffs/softcenter/configs/frps.ini"
	cat >${INI_FILE} <<-EOF
	...
	if [ "${frps_common_cron_time}" == "0" ]; then
		...
		if [ "${frps_common_cron_hour_min}" == "min" ]; then
			...
			cru a frps_monitor "*/"${frps_common_cron_time}" * * * * /bin/sh /jffs/softcenter/scripts/frps_config.sh"
		elif [ "${frps_common_cron_hour_min}" == "hour" ]; then
			echo_date "设置定时任务：每隔${frps_common_cron_time}小时注册一次frps服务..."
			cru a frps_monitor "0 */"${frps_common_cron_time}" * * * /bin/sh /jffs/softcenter/scripts/frps_config.sh"
		fi
		echo_date "定时任务设置完成！"
	fi
